...
4. Copies `sqlite3.dll` from vcpkg. The repo uses **manifest mode** (`vcpkg.json`), so the script searches `<BuildDir>\vcpkg_installed\<triplet>\bin`, then `<RepoRoot>\vcpkg_installed\<triplet>\bin`, then the classic-mode `%VCPKG_ROOT%\installed\<triplet>\bin` — triplet is `VCPKG_DEFAULT_TRIPLET` or `x64-windows`.
