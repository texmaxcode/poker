...
- **`poker/main.cpp`** sets `QCoreApplication` organization/name for **`QSettings`**, creates **`game`**, calls **`loadPersistedSettings()`**, instantiates **`PokerSolver`** and **`SessionStore`**, and exposes **`pokerGame`**, **`pokerSolver`**, and **`sessionStore`** to QML. **`aboutToQuit`** saves table-related settings.
